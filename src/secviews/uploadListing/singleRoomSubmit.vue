...
<script>
    import {addHomestays} from "../../utils/api";
    ...
    export default {
        ...
        methods:{
            onChange(){
                ...
                let uploadData = {
                    landlord_id:7,
                    homestay_name:this.form.name,
                    homestay_addr:this.form.region,
                    homestay_pic:this.form.pic,
                    homestay_cap:this.form.clientNum,
                    homestay_area:this.form.area,
                    homestay_info:this.form.desc,
                    homestay_price:this.form.price,
                    homestay_status:1,
                    by_the_street:"房间正临街"===this.form.surrounding?1:0,
                    wifi:"店内有WIFI"===this.form.equipment?1:0,
                    bathtub:-1,
                    bed_number:this.form.bedNum,
                    WholeRoom:null,
                    SingleRoom:null
                }
                if(this.$route.query.type === 'all'){
                    uploadData.WholeRoom =
                        {
                            room_num:this.tableData.length,
                            Room:[]
                        }
                    for (let i = 0; i < this.tableData.length; i++) {
                        uploadData.WholeRoom.Room.push({
                            room_picture:this.tableData[i].pic,
                            room_info:this.tableData[i].desc
                        })
                    }
                }
                else {
                    uploadData.SingleRoom=[]
                    for (let i = 0; i < this.tableData.length; i++) {
                        uploadData.SingleRoom.push({
                            bed_type:this.tableData[i].tag==="单人床"?0:1
                        })
                    }
                }
                console.log(uploadData)
                addHomestays(uploadData).then(res=>{
                    console.log(res)
