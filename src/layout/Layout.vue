...
<!--        roles === user-->
        <el-menu-item v-if="menu.children && menu.children.length === 1 && menu.children[0].meta.roles === this.roles" :index="menu.children[0].path" >
...
export default {
  ...
  setup(props, ctx){
    ...
    let roles = ref(store.state.userInfo.userRole)
    return{
      ...toRefs(state),
      onCollapse,
      onRefresh,
